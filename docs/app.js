...
// ── Programming lesson content
    ...
    ${section('Data Compression',
      ...
      p('Example: <strong>Run-Length Encoding (RLE)</strong>: consecutive repeated values are stored as (count, value) pairs.'),
      `<div class="code-block"><code>Original:  AAAAABBBBCCDAA   (13 characters)
Encoded:   5A 3B 2C 1D 2A  → 5A3B2C1D2A  (10 characters)</code></div>`,
      p('Calculation: 13 × 8 = <strong>104 bits</strong> → 10 × 8 = <strong>80 bits</strong>. That is a <strong>23% reduction</strong> in size.'),
      tip('RLE is effective on simple images with large areas of the same colour (logos, icons). It may <em>increase</em> file size for complex photographic images with few repeating patterns: for example, a portrait with thousands of unique colours would produce a longer encoded string than the original.'),
      ...
      h3('Transform Coding (JPEG)'),
      p('JPEG uses <strong>transform coding</strong>: a four-stage lossy compression process:'),
      `<div class="tbl-wrap"><table class="content-table">
        <thead><tr><th>Stage</th><th>What happens</th></tr></thead>
        <tbody>
          <tr><td><strong>1. Sub-images</strong></td><td>The full image is divided into small n × n blocks (typically 8 × 8 pixels)</td></tr>
          <tr><td><strong>2. Forward transform (DCT)</strong></td><td>Each block is converted from the spatial domain (pixel values) to the frequency domain using the Discrete Cosine Transform, producing low-frequency and high-frequency coefficients</td></tr>
          <tr><td><strong>3. Quantizer</strong></td><td>High-frequency coefficients (fine detail) are rounded or discarded: the human eye is less sensitive to high-frequency detail, so this loss is barely perceived. This is the lossy step</td></tr>
          <tr><td><strong>4. Symbol encoder</strong></td><td>The quantized data is further losslessly compressed using: zigzag scan → RLE → Huffman coding</td></tr>
        </tbody>
      </table></div>`,
      examTip('JPEG is lossy because the quantizer permanently discards high-frequency data. The original cannot be recovered. This is why JPEG must not be used for text, medical scans, or any data where every detail matters.'),
      `<div class="tbl-wrap"><table class="content-table">
        <thead><tr><th></th><th>Lossless</th><th>Lossy</th></tr></thead>
        <tbody>
    ...
    ${section('Practice Questions',
      practiceSect('Questions with Model Answers', [
        qa('What are the two main advantages of compressing files?',
          '<p>1. Compressed files take up less space on secondary storage.<br>2. They are faster to transmit across a network.</p>'),
        qa('Explain the difference between lossless and lossy compression, with an example of each.',
          '<p><strong>Lossless:</strong> Data is compressed but can be perfectly restored. No information is discarded. Example: PNG images, ZIP files, FLAC audio. Used for text and databases where accuracy is critical.<br><strong>Lossy:</strong> Some data is permanently removed. The original cannot be recovered. Example: JPEG images, MP3 audio. Acceptable for media where small quality reductions are imperceptible.</p>'),
        qa('How does run-length encoding (RLE) work? Encode the string AAAAABBBBCCDAA and calculate the space saving.',
          '<p>RLE replaces runs of repeated characters with (count, character) pairs.<br><br>AAAAABBBBCCDAA → <code>5A3B2C1D2A</code><br>Original: 13 chars × 8 bits = 104 bits<br>Encoded: 10 chars × 8 bits = 80 bits<br>Saving: 24 bits → approx. 23% reduction.<br><br>Note: RLE is less effective on data with few repeating patterns and can increase file size for complex images.</p>'),
        qa('Describe the four stages of transform coding in JPEG compression and explain why it is lossy.',
          '<p>1. <strong>Sub-images:</strong> Image divided into 8 × 8 pixel blocks.<br>2. <strong>DCT (forward transform):</strong> Each block converted from pixel values (spatial domain) to low and high frequency coefficients (frequency domain).<br>3. <strong>Quantizer:</strong> High-frequency coefficients are reduced in precision or discarded: the lossy step, because the human eye is less sensitive to fine detail.<br>4. <strong>Symbol encoder:</strong> The result is further compressed losslessly using zigzag scan, RLE, and Huffman coding.<br><br>It is lossy because the quantizer permanently removes data that cannot be recovered.</p>'),
        qa('When would you choose an external SSD over an external HDD?',
          '<p>Choose an SSD when speed, portability, and durability matter: quick file transfers, backups, or carrying the drive around. Choose an HDD when you need large capacity at low cost and speed is less critical, such as archiving large media collections.</p>'),
        qa('What advantages does NAS offer for a business environment?',
          '<p>NAS provides centralised data storage accessible to multiple users over a network. Key advantages: high capacity (multiple HDDs/SSDs in RAID configuration), redundancy through RAID (data is not lost if one drive fails), easy expansion by adding drives, and centralised backup management: all connected via Ethernet.</p>'),
        qa('Why might RLE increase file size for a portrait photograph?',
          '<p>A portrait photograph contains millions of unique pixel colours with very few repeating consecutive values. RLE replaces each single unique pixel with a count-value pair (e.g. <code>1R</code> instead of just <code>R</code>), which doubles the storage required for non-repeating sections: resulting in a larger encoded file than the original.</p>'),
      ]),
      practiceSect('Student Practice', [
        qa('Explain why lossless compression is essential for text files and databases, while lossy compression is acceptable for images and videos.', ''),
        qa('A file is 2.4 MB. After lossy compression it is 600 KB. Calculate the compression ratio and the percentage reduction.', ''),
        qa('Describe a programming task: write an RLE compress/decompress application. The compress function receives a string and outputs the encoded version; decompress does the reverse.', ''),
      ])
    )}`;
  ...
  case 'l5': return `
    ...
    ${section('Practice Questions',
      practiceSect('Questions with Model Answers', [
        qa('What is the primary function of a GPU, and how does its architecture differ from a CPU?',
          '<p>A GPU accelerates the rendering of images, videos, and animations by performing rapid parallel calculations. Unlike a CPU, which has a few powerful cores optimised for sequential, complex logic, a GPU has <strong>thousands of smaller cores</strong> designed to execute many simple operations simultaneously (parallel processing). This makes GPUs far faster for tasks that can be broken into many concurrent operations.</p>'),
        qa('How have GPUs evolved beyond graphics rendering?',
          '<p>In the early 2000s, researchers realised that GPU parallel architecture suited many other computationally intensive tasks. Today GPUs are widely used for:<br>• <strong>Machine learning / AI</strong>: neural networks rely on matrix and vector multiplication, which GPUs excel at<br>• <strong>Scientific simulations</strong>: weather, fluid dynamics, physics<br>• <strong>Video processing</strong>: encoding/decoding 4K streams<br>• <strong>Cryptocurrency mining</strong>: parallel cryptographic hashing<br>Multiple GPUs can also run in parallel (GPU clusters) to further accelerate large workloads.</p>'),
        qa('Why are GPUs significantly faster than CPUs for machine learning tasks?',
          '<p>Machine learning models (especially neural networks) rely heavily on <strong>matrix and vector multiplications</strong>. These involve applying the same simple arithmetic operation to millions of values simultaneously. GPUs, with thousands of cores built for parallel computation, process these in parallel. A CPU\'s few powerful cores handle them sequentially: far slower for this workload.</p>'),
        qa('What is Software as a Service (SaaS) and how does it differ from traditional software installation?',
          '<p>SaaS delivers software over the internet via a web browser: no local installation required. Differences from traditional installation:<br>• <strong>Access:</strong> From any device with internet connectivity<br>• <strong>Maintenance:</strong> Provider handles updates and security<br>• <strong>Cost:</strong> Usually subscription-based rather than one-time licence purchase<br>• <strong>Dependency:</strong> Requires internet connection to use<br>Example: Google Workspace (Gmail, Docs, Drive)</p>'),
        qa('How does Platform as a Service (PaaS) benefit software developers?',
          '<p>PaaS provides a ready-made cloud platform (databases, middleware, development frameworks) so developers can focus on writing application code rather than managing servers. Benefits: faster development, easy horizontal scaling as users grow, and reduced infrastructure costs. Drawback: vendor lock-in and less control over the hosting environment. Example: Microsoft Azure App Service.</p>'),
        qa('Why might a business choose IaaS over purchasing physical servers?',
          '<p>IaaS (e.g. AWS EC2) provides virtualised computing resources (VMs, storage, networks) on a pay-as-you-go basis. Benefits: <strong>no upfront hardware cost</strong>, scalability to grow or shrink as needed, and <strong>full control</strong> over VMs and operating systems. Drawback: requires more technical knowledge than PaaS, as the business must manage its own security and software stack.</p>'),
      ]),
      practiceSect('Student Practice', [
        qa('Describe how the parallel structure of GPUs makes them suitable for machine learning tasks compared to CPUs.', ''),
        qa('What are the main advantages and disadvantages of using SaaS for business applications?', ''),
        qa('Compare PaaS and IaaS in terms of control, flexibility, and use cases. When would a company choose one over the other?', ''),
      ])
    )}`;
